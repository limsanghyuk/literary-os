...
BASE = Path('handoff/20260911/i4k5r2_attempt2')
FORBIDDEN = ['Event Ecology', 'thread', 'engine', 'planner', 'Treatment', 'Control', 'threshold', 'goal', 'obstacle', 'exit_state', 'downstream_decision_owner', 'future_thread_owner']
...
def body_from_segment(text):
    ...
    end = text.rfind('[END ATTEMPT2')
    if end < 0:
        raise RuntimeError('missing attempt2 end marker')
    return text[m.start():end].rstrip('\n')

def measure_arm(arm):
    root = BASE / arm.lower()
    files = sorted(root.glob(f'P07_I4K5R2_A2_{arm}_SQ*_SC*.txt'))
    rows = []
    bodies = []
    scene_ids = []
    for p in files:
    ...
    body_chars = sum(len(x) for x in bodies)  # excludes transport metadata and synthetic join chars
    return {
...
print('I4K5R2_ATTEMPT2_METRICS=' + json.dumps(out, ensure_ascii=False, sort_keys=True))
