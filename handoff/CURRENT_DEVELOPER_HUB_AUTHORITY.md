...
Read together with `handoff/CURRENT_SESSION_RECOVERY_POINTER.md`. Both describe the same authority.
...
`e64fb3ff9fa120ed8d784f6544bf97e3961ed4d4`
...
## P07-I4A SPLIT STATUS
### Provider operating track — PASS
Canonical live semantic provider construction now uses `build_live_semantic_provider()` and bounded fail-closed `ResilientSemanticProvider` with default max attempts 2. Transient 429/500/503, incomplete, timeout/network and provider JSON failures may retry; HTTP 400/non-transient failures do not retry. `X-Client-Request-Id`, per-attempt traces, provider-claim blocking on model mismatch/missing response-id, and no State Commit after planning failure are verified. Exact packaged-C2 nonhistorical regression: 205/205 PASS, exit code 0.

### Literary surface track — HOLD
Calibrated human blind baseline: Human 12 / Candidate 0 / Tie 0. Six-scene literary-compression/voice repair: Human 5 / Candidate 1 / Tie 0. Preregistered Candidate wins+ties >=3/6 gate failed at 1/6, so `SURFACE_REPAIR_INSUFFICIENT__HOLD_FULL_EPISODE_RERENDER`. The failed six-scene intervention remains research evidence only and is not active engine policy.
...
## CLAIM BOUNDARY
P07-I4A physically adopts Provider Shadow Live resilience/fail-close improvements only. It does not promote the failed six-scene literary surface repair and does not establish a real OpenAI Live call, human broadcast-writer craft equivalence, full-episode surface repair, RFV3, CP1 Live, official R-F/R-G, Production promotion, or Formal R140.

## NEXT ORDER
Use these exact nine packages as the sole starting authority. Redesign the `Scene Plan -> Surface Realization` interface itself rather than relaxing blind thresholds. Run a small causal and blind probe before any full 50-scene rerender. Priority: character-specific voice state, dialogue-information budgets, relationship-specific behavior, subtext/physicalization, and stronger scene-level literary compression.
