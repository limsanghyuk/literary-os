...
This dossier is the detailed next-session reference for:
- Claude/CT defects;
- working-state repairs;
- still-open defects;
- exact developer-held previous 5-Part / 9-Package baseline hashes;
- C2-A + C2-B reconstruction contract;
- the critical limitation that repaired source bytes are not yet confirmed as a complete durable code snapshot;
- controlled repair reconstruction/reverification if exact repaired bytes do not survive;
- RFV2 purpose/content/method/results boundary;
- RFV3 A/B/C/D purpose/content/method;
- CP1 role and restoration requirements;
- exact package rebuild/audit/resume order.
...
These values are previous baseline only and must be verified from uploaded files before use:
...
The previous physical 9-package hashes are PREVIOUS_PHYSICAL_BASELINE only. Do not promote them to current repaired authority until repaired bytes are physically rebuilt and audited.
