...
2. `handoff/20260909/P07_POST_DELIVERY_RUNTIME_TRANSPORT_RECURRENCE_R1_20260909.md`
...
## RECOVERY STATUS
P07-I4H Recovery R3 physical closure is complete, but the execution-transport layer remains an intermittent external operational risk.

In the healthy recovery runtime before the post-delivery recurrence:
...
After the R3 package had already been built, independently verified, delivered, and Hub-closed, two consecutive container probes again returned `TransportTimeoutError`, including a minimal `/bin/true`. This does not invalidate R3 package bytes; it means new sessions must perform runtime preflight before new research.
...
- platform/tool execution transport instability remains intermittent; a post-delivery recurrence produced `TransportTimeoutError` twice, including on minimal `/bin/true`;
- a generated-file durability boundary error (`GeneratedFileUploadError`) was also reproduced after local filesystem work had completed, supporting a tool/platform transport-layer failure class for at least part of the historical instability;
- the container has a hard cgroup 4 GiB memory limit and zero swap; large sequential archive reads filled page cache and hit `memory.events max`, but `oom=0` and `oom_kill=0` throughout the completed recovery;
...
Before studying, analyzing, or modifying the nine R3 packages in a fresh session:
1. mount all nine exact R3 transport files;
2. run a minimal execution probe;
3. run small filesystem read/write/stat checks;
4. inspect cgroup memory state if available;
5. verify one small ZIP member read;
6. then verify all nine outer SHA256 values by streaming;
7. only after those checks pass may large-package analysis or new research begin.

If `TransportTimeoutError`, `ClientError`, or generated-file durability failure occurs, stop new research and retain the last sealed R3 authority.
...
The next research phase may resume only from this R3 physical authority and the already durable I4I preregistration/plan material, and only after the mandatory new-session runtime preflight passes. I4I still has no Control, selector, Treatment or score output.

## STATUS TOKEN
`I4H_RECOVERY_R3_PHYSICAL_CLOSURE_COMPLETE__SYNC_R6_PARENT_9_9__I4D_213_213__R2_HOLD_44_45__R3_45_45__TARGETED_56_56__FULL_258_258__PACKAGE_SET_9327630E__FINAL_C2_58D28ECC__POST_DELIVERY_TRANSPORT_TIMEOUT_RECURRED__NEW_SESSION_PREFLIGHT_REQUIRED__DB59_FROZEN__PRODUCTION_R47__FORMAL_137__R140_0_0_0__I4I_0_0_0_0`
