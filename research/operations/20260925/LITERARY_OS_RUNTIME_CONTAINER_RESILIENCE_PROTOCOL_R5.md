# Literary OS Runtime / Container Resilience Protocol R5
...
## 20. Nested authority-shadowing protection

A sealed physical package may intentionally contain older recovery overlays whose embedded files are named `CURRENT_*`.
Those names do not by themselves establish current authority.

For recursive recovery, the mandatory precedence is:
1. external current physical Manifest;
2. external current Trust Root;
3. external current READ FIRST;
4. top-level current-authority metadata inside the current package;
5. Hub CURRENT handoff/session pointers;
6. nested historical overlays and their embedded CURRENT pointers.

A nested archive whose own manifest declares `physical_authority_change=false` cannot supersede the enclosing sealed physical authority.

Current concrete case:
- enclosing authority: SYNC-R73;
- nested B2 object: `POST_SYNC_R72_FULL_RECOVERY_OVERLAY_R3.zip`;
- nested object SHA256: `e215795bdcafacb20ed4ee91f009cbd24c87dc8ebb07c6664e86056d7c1bc72f`;
- nested CURRENT pointers correctly describe historical SYNC-R72 and are provenance-only.

Do not delete or rewrite historical evidence to remove this ambiguity.
Use `SYNC_R73_RECOVERY_PRECEDENCE_GUARD_R1`.

## 21. SYNC-R73 integrity/debug closure

Direct audit result:
`CLOSED_PASS__9_OF_9_INTEGRITY__NO_PACKAGE_CORRUPTION__CLIENTERROR_LOCALIZED__AUTHORITY_SHADOWING_GUARD_ACTIVE`

Established:
- all 9 transport size/SHA match the sealed manifest;
- all 7 ZIP carriers pass central-directory safety and CRC;
- logical C2 rejoin passes exact size/SHA/CRC;
- DB64-R131 embedded split rejoin matches canonical full DB;
- H1 Past-Only packages match frozen hashes and contain no hidden target payload;
- no parent entry was unintentionally deleted or changed;
- intermittent ClientError is not supported as OOM or package corruption;
- grouped long-call timeout is prevented by atomic one-artifact verification;
- nested authority-shadowing is resolved by precedence guard without physical reseal.
