...
# Insert from high scene numbers downward so offsets remain safe.
for sc in sorted([int(k) for k in ins], reverse=True):
    start=re.search(rf"(?m)^씬\s+{sc}\.\s*[^\n]*$",text)
    if not start: raise SystemExit(f"scene start missing {sc}")
    if sc<50:
        nxt=re.search(rf"(?m)^씬\s+{sc+1}\.\s*[^\n]*$",text[start.end():])
...
res={"schema":"R77_H1_PA7_CONTROL_R3B_MECHANICAL_RESULT","date":"2026-09-26",
...
(D/"R77_H1_PA7_CONTROL_R3B_FULL_SCREENPLAY.txt").write_text(text,encoding="utf-8")
(D/"R77_H1_PA7_CONTROL_R3B_MECHANICAL_RESULT.json").write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding="utf-8")
