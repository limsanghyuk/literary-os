...
═══════════════════════════════════════════
## 7. 최신 업데이트 (2026-06-07 후반)
═══════════════════════════════════════════
**추가 실행·발견 (커밋 8c63029→e79acd0):**
1. **재가중 held-out 교차검증 → 이전 주장 교정**: '재가중 0.40→0.59 개선'은 과적합 아티팩트(작품단위 LOO held-out 효과 +0.002). **기존 fitness 가중치는 held-out 0.70으로 잘 일반화(공식 미파손)**. → 하니스가 과대주장을 스스로 적발.
2. **트라이스토어↔공식 하니스 연결 동작**(피처테이블 SQL→실 공식→상관, N=30).
3. **Gold20 후보 + 합법 스크립트 가용성**: 경로B 대본집 확인 4편(비밀의숲·미생·우영우·우리들의블루스), 경로A 폭/B 깊이 분담.
4. **P1~P5 완성도 감사**: 대부분 설계도(L3)까지. 갭 = P2 인간GT 프로토콜·피처테이블 정본스키마 / P3 Stage3~6 / P4 구현명세 / P5 상세.
5. **레퍼런스 기반 검증 프로토콜(개발자 제안 채택)**: 실제 시나리오=GT 레퍼런스, 다중에이전트=인간 대행 → proxy 순환 해소. Mode2(공식 on 실제, 순환0 우선)/Mode1(생성 vs 실제 블라인드).
6. **refcheck POC(OpenAI 실행)**: 다중에이전트 블라인드 작동(3:0). **단 LLM 회상 레퍼런스는 무효 입증** → Mode1·Mode2 모두 **합법 대본집(실제 원문)이 비협상 전제**임이 실측 확인.

**키 상태**: Anthropic 미충전, Gemini 소진, **OpenAI 가용**(gpt-4o/4o-mini로 실험 가능).

**최우선 액션(불변)**: 개발자가 **대본집 4편 입수** → Mode2(공식 on 실제 명작, 순환0) 우선 실행 → 공식 절대 타당성 1차 검증. 이게 모든 검증의 진짜 관문.

**현 HEAD**: e79acd0 (본 업데이트 push 후 갱신)
