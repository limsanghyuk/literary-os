...
targets=[w for w,e in nkg.items() if not e.get("characters")]
def sample(t): return t[:6000]+"\n...\n"+t[len(t)//2:len(t)//2+4000]
def ask(txt):
...
for w in targets:
    ...
    try:
        raw=ask(txt); m=re.search(r'\[.*\]',raw,re.S); names=json.loads(m.group(0)) if m else []
    except Exception as ex: print("ERR",w,str(ex)[:40],flush=True);continue
    grounded=[]
    for nm in names:
        nm=str(nm).strip()
...
rem=sum(1 for w,e in nkg.items() if not e.get("characters"))
print(f"this run:{done} | 잔여 NOCHAR:{rem}",flush=True)
