...
### V790 — 설계 문서(코드 무변경)
- `docs/design/DESIGN-DATA-EVAL-DELIBERATION-v1.md` 신규: 개발자 지적 3·4·5 비판검토(무조건수용 차단)
- BLOCK: 데이터 스케일 선행(틀린 신호 증폭) / CORRECT: 씬·회차 DPO로 아크 목표 학습불가(단위 불일치) / BLOCK: 0.357 자기모순(무효지표 결론) / ELEVATE: 암기·표절=하드 안전게이트
- 3인(ARCH×COMP×DATA) 교차논의 + SPE 감독 확정: 페어링 P1/P3/P2/P4, 스케일 2k/held250, 평가 E1~E5(E1/E4 전수 결정론)
- LADDER §3.3 데이터·평가 빈칸 채움 + 광역목표 계층플래너 트랙 이관

---

## [13.42.0] — V789 — LLM 자율성 사다리(LLM-0~3) 상세 설계도 (DESIGN-LLM-LADDER-v1)

### V789 — 설계 문서(코드 무변경)
- `docs/design/DESIGN-LLM-LADDER-v1.md` 신규: LLM-0/1/1.5/2/2.5/3 6단계 정의 + 진급 게이트 **수치 계약**
- L1→L2 졸업 계약(§3.3) 신규 제시: adopt≥5라운드·Σn_pairs≥250·per-token W CI하한>0.5·길이단순규칙 재현율≤0.60
- 게이트↔코드 매핑(현존/V788신규/빈칸) + 신규 ADR-LADDER-1~5 제안 + Phase E/F/G 정합
- 자기점검: 임계는 미실측 제안값(실 GPU ΔW·인간평가 후 캘리브레이션 필수)
...
- `critic/self_eval_pipeline.py` SelfEvalPipeline: M1 자격→M2 선호쌍→M3 가드→loop-C 입력
- M1 미통과 전체 차단. 병리 생성=rejected 유지(부정신호)·이긴 모순쌍 제외
- 인간없이 명작 닻으로 학습신호 생성 사슬 완결. 12 TC. 13.37→13.38

---

## [13.37.0] — V784 — M3 분포 음성 가드레일 (ADR-245)

### V784 — 자체평가 3종 완성 (M3)
- `critic/distribution_guard.py`: 명작 통계 밴드 밖(병리)만 감점, 밴드 안=0(전형성 무보상)
- 평균회귀·굿하트 차단(비대칭: 깨진것 거르되 전형성 무보상). 한국어 단문체 보정
- 13 TC. M1(자격)+M2(보상닻)+M3(가드) 완성. 다음 loop-C 통합. 13.36→13.37

---

## [13.36.0] — V783 — M2 NextEpisodeBench: 은닉 GT 쌍대 (ADR-244)

### V783 — 자체평가 보상 주 닻 (M2)
- `critic/next_episode_bench.py`: N+1화 생성 vs 실제 은닉분 쌍대(필적), 위치스왑, loop-C 선호쌍
- 누출가드(유명작 제외·n-gram 중첩>0.25 제외), M1 선결 강제(자격 critic만 심판)
- 14 TC. 다음 M3 분포 음성가드. 13.35→13.36
...
### V780 — 모듈레벨 순환 검출
- `tools/run_preflight.py` `_build_module_level_deps()`: 함수 내부 지연 import 제외 → 순환 검출 정밀화
- 순환 3건(전부 lazy import 오탐) 해소, 경고 8→5. 연결성·영향분석 불변. 5 TC. 13.32→13.33
